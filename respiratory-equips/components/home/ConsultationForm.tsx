    ...
    <section id="contact" className="max-w-3xl mx-auto px-6 lg:px-8 pb-16">
      <div className="bg-white rounded-2xl shadow-md border border-gray-100 p-6 md:p-8">
        ...
        <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <Field icon={FaUser} required type="text" placeholder="Full Name*" />
          <Field icon={FaPhoneAlt} required type="tel" placeholder="Phone Number*" />
          <Field icon={FaEnvelope} required type="email" placeholder="Email Address*" />
          <Field icon={FaMapMarkerAlt} required type="text" placeholder="City*" />

          <div className="relative">
            <FaStethoscope className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400" />
            <select className="w-full appearance-none rounded-lg border border-gray-200 bg-white pl-11 pr-4 py-3 text-sm text-gray-600 outline-none focus:border-accent focus:ring-2 focus:ring-accent-light transition-colors">
          ...
          <label className="relative flex items-center rounded-lg border border-dashed border-gray-300 bg-gray-50 pl-11 pr-4 py-3 text-sm text-gray-500 cursor-pointer hover:border-accent hover:bg-accent-light/40 transition-colors">
            ...
            <span className="flex-1 truncate">Upload Prescription</span>
          ...
          <button
            type="submit"
            className="md:col-span-2 flex items-center justify-center gap-2 bg-accent hover:bg-accent-dark text-white rounded-lg py-3.5 font-semibold transition-colors shadow-sm"
          >
            Request Callback <FaPaperPlane />
          </button>
        </form>
      </div>
    </section>
